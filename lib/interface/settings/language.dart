...
class LanguageSetting extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final List<LanguageRegion> regions = LanguageRegion.values.toList()
      ..removeWhere((languageRegion) =>
      languageRegion == context
          .read(langageProvider)
          .current)
      ..insert(0, context
          .read(langageProvider)
          .current);
    return Consumer(
      builder: (context, ScopedReader watch, _) {
        var language = watch(langageProvider);
        return Card(
  ...
  Widget _buildLanguageRegionTile(LanguageRegion languageRegion, {Key? key}) {
    return Consumer(
      ...
      return RadioListTile<LanguageRegion>(
        ...
        onChanged: (languageRegion) async => (languageRegion != null ? (await language.set(languageRegion: languageRegion)):null),
        secondary: Container(
          height: 20,
